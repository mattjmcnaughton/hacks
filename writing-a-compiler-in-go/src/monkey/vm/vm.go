...
func (vm *VM) executeArrayIndex(array, index object.Object) error {
	// Can perform type assertion without checking `ok`, because we've
...
func (vm *VM) executeHashIndex(hash, index object.Object) error {
	...
	// Check if key conforms to the hash interface
...
func (vm *VM) executeComparison(op code.Opcode) error {
	...
	switch op {
	case code.OpEqual:
		// We have a single `true` and `false` object that we reuse, so
		// we can compare `right == left` and, even thought that
		// compares object equality, it will still function as we hope.
...
func (vm *VM) executeIntegerComparison(op code.Opcode, left, right object.Object) error {
	// We know they are object.Integer because we checked their Type... but
	// we still need to cast them.
	leftValue := left.(*object.Integer).Value
	rightValue := right.(*object.Integer).Value
...
func (vm *VM) callBuiltin(builtin *object.Builtin, numArgs int) error {
	// Unlike the compiled function, we need to fetch the args. For a
	// compiled function, the args are available via local bindings.
